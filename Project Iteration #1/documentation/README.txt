...
the client, server, and intermediate host. Additional consoles can be opened with the "Open Console" option 
...
SERVER_PORT_NUMBER:
	...
	NOTE: if this value is changed in the intermediate host, it must be changed to the same value in the server
		  as well!
INTERMEDIATE_HOST_PORT_NUMBER:
	This integer value is the port number that the intermediate host will be found at.
...
DESIGN CHOICES
---------------
While all three of the client, server, and intermediate host have many of the same functionality (send, receive, many 
identical constants), the decision was made to not use a superclass/interface to reduce this repetition. 

This is because these are three separate programs which theoretically should be able to run on different computers with 
